...
/* Profile photo migration/support */
function collectLocalPhotos(){
  const found=[];const seen=new Set();
  function addPlant(p){if(!p||typeof p!=='object'||typeof p.photo!=='string'||!p.photo.startsWith('data:'))return;const key=String(p.id||'')+'|'+String(p.type||'')+'|'+String(p.nickname||'');if(seen.has(key))return;seen.add(key);found.push({id:p.id||'',type:p.type||'',nickname:p.nickname||'',photo:p.photo});}
  ...
  for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(!k)continue;let parsed;try{parsed=JSON.parse(localStorage.getItem(k)||'null');}catch(e){continue;}if(Array.isArray(parsed))parsed.forEach(addPlant);else if(parsed&&typeof parsed==='object'){addPlant(parsed);Object.values(parsed).forEach(v=>{if(Array.isArray(v))v.forEach(addPlant);});}}
  return found;
}
function matchCapturedPhoto(p){let m=capturedLocalPhotos.find(x=>x.id&&String(x.id)===String(p.id));if(!m)m=capturedLocalPhotos.find(x=>x.type===p.type&&x.nickname===p.nickname&&x.photo);return m||null;}
async function uploadCapturedPhotos(){
  if(!cloudUser||!capturedLocalPhotos.length||syncing)return;
  syncing=true;
  try{let uploaded=0;for(const local of capturedLocalPhotos){let target=null;if(local.id)target=plants.find(p=>String(p.id)===String(local.id));if(!target)target=plants.find(p=>p.type===local.type&&p.nickname===local.nickname);if(!target||!local.photo||!local.photo.startsWith('data:'))continue;const safe=safeSegment(target.id||Date.now());const path=await uploadBlobAt(local.photo,cloudUser.id+'/'+safe+'/main');const updated=Object.assign({},target,{photo:'',photoPath:path});const {error:updateError}=await sb.from('plantdex_plants').update({photo_path:path,data:updated}).eq('local_id',String(target.id));if(updateError)throw updateError;target.photoPath=path;target.photo=await signedPhoto(path);uploaded++;}if(uploaded){originalSavePlants();if(typeof render==='function')render();status('☁️ Photos synced');}}
  catch(e){console.error('Plantdex captured photo upload failed',e);status('⚠️ Photo sync issue — local photo kept');}
  finally{syncing=false;}
}
async function uploadPhoto(p){if(!p.photo||!p.photo.startsWith('data:'))return p.photoPath||'';const path=await uploadBlobAt(p.photo,cloudUser.id+'/'+safeSegment(p.id||Date.now())+'/main');p.photoPath=path;p.photo=await signedPhoto(path);return path;}

/* Timeline/gallery cloud storage. The gallery data structure is preserved; image leaves are
   replaced by blanks in Postgres and their Storage paths are kept in a parallel tree. */
async function prepareGalleryNode(node,oldPaths,stem){
  ...
  if(Array.isArray(node)){
    const clean=[],paths=[];
    for(let i=0;i<node.length;i++){const r=await prepareGalleryNode(node[i],Array.isArray(oldPaths)?oldPaths[i]:null,stem+'/'+i);clean.push(r.clean);paths.push(r.paths);}
    return {clean,paths};
  }
  if(node&&typeof node==='object'){
    const clean={},paths={};
    for(const [k,v] of Object.entries(node)){const r=await prepareGalleryNode(v,oldPaths&&typeof oldPaths==='object'?oldPaths[k]:null,stem+'/'+safeSegment(k));clean[k]=r.clean;paths[k]=r.paths;}
    return {clean,paths};
  }
  return {clean:node,paths:null};
}
...
async function hydrateGalleryNode(node,paths){
  ...
  if(Array.isArray(node)){const out=[];for(let i=0;i<node.length;i++)out.push(await hydrateGalleryNode(node[i],Array.isArray(paths)?paths[i]:null));return out;}
  if(node&&typeof node==='object'){const out={};for(const [k,v] of Object.entries(node))out[k]=await hydrateGalleryNode(v,paths&&typeof paths==='object'?paths[k]:null);return out;}
  return node;
}
...
async function syncNow(){
  ...
  try{
    for(const p of plants){
      ...
      const clean=Object.assign({},p,{photo:'',photoPath:path||p.photoPath||'',gallery:galleryResult.clean,galleryCloudPaths:galleryResult.paths});
    ...
    originalSavePlants();if(typeof render==='function')render();status('☁️ Synced');
...
async function loadCloud(){
  status('☁️ Loading…');capturedLocalPhotos=collectLocalPhotos();
  const {data,error}=await sb.from('plantdex_plants').select('local_id,data,photo_path').order('created_at');if(error)throw error;
  if(data.length){
    plants=await Promise.all(data.map(async row=>{let p=Object.assign({},row.data||{},{id:row.local_id});p.photoPath=row.photo_path||p.photoPath||'';p.photo=await signedPhoto(p.photoPath);p=await hydrateGallery(p);const local=matchCapturedPhoto(p);if(local&&!p.photoPath)p.photo=local.photo;return p;}));
    await uploadCapturedPhotos();originalSavePlants();render();if(!capturedLocalPhotos.length)status('☁️ Synced');
...
function schedulePhotoScan(){[300,1000,2500].forEach(ms=>setTimeout(async()=>{if(!cloudUser||syncing)return;capturedLocalPhotos=collectLocalPhotos();if(capturedLocalPhotos.length)await uploadCapturedPhotos();if(!syncing)await syncNow();},ms));}
