...
function renderApp_(plants, initialPlantId, canEdit) {
  ...
  var body = [
    ...
    '    <h1 class="text-3xl font-bold text-[#2d5a3d] tracking-tight" style="letter-spacing:-0.04em">Plantdex</h1>',
    '  </div>',
    '  <button',
    '    x-show="canEdit"',
    '    x-cloak',
    ...
    '    style="transition:.18s ease" onmouseover="this.style.transform=\'translateY(-1px)\'" onmouseout="this.style.transform=\'\'">',
    '    ＋ Add Plant',
    '  </button>',
    '</div>',
    ...
    '  <!-- Total -->',
    '  <div class="bg-white rounded-[18px] border border-[#e8f0ea] p-4" style="box-shadow:0 14px 34px rgba(32,67,49,.10)">',
    '    <div class="text-3xl font-black text-[#2d5a3d]" x-text="stats.total"></div>',
    '    <div class="text-sm text-[#5a7a65] mt-0.5">Total Plants</div>',
    '  </div>',
    '  <!-- Top 3 categories -->',
    '  <template x-for="cat in stats.cats" :key="cat.name">',
    '    <div class="bg-white rounded-[18px] border border-[#e8f0ea] p-4" style="box-shadow:0 14px 34px rgba(32,67,49,.10)">',
    '      <div class="text-3xl font-black text-[#2d5a3d]" x-text="cat.count"></div>',
    '      <div class="text-sm text-[#5a7a65] mt-0.5 truncate" x-text="cat.name"></div>',
    '    </div>',
    '  </template>',
    '</div>',
    ...
    // ── INDEX TABLE ──────────────────────────────────────────────────────────
    '<div class="overflow-hidden rounded-2xl border border-[#e8f0ea] bg-white" style="box-shadow:0 14px 34px rgba(32,67,49,.10)">',
    ...
    '  style="background:rgba(15,31,23,.5);backdrop-filter:blur(3px)"',
    '  @click.self="close()"',
    '  @keydown.escape.window="close()"',
    '>',
    '  <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" style="box-shadow:0 25px 80px rgba(0,0,0,.25)">',
    ...
    // Badges row — Category and Leaf Watch pills
    '      <div class="flex flex-wrap gap-2 px-6 pb-4 pt-4">',
    '        <span x-show="current && current.Category"',
    '              x-text="current && current.Category"',
    '              class="rounded-full px-3 py-1 text-xs font-semibold"',
    '              style="background:#dfeee5;color:#2f6b4f"></span>',
    '        <span x-show="current && current[\'Leaf \ud83d\udc40\']"',
    '              class="rounded-full px-3 py-1 text-xs font-semibold"',
    '              style="background:#e3f3e8;color:#28603f">\ud83d\udc41 Leaf Watch</span>',
    '      </div>',
    ...
    // VIEW footer
    '      <div class="flex items-center gap-3 px-6 pt-4 pb-4 mt-2 border-t border-[#e8f0ea]">',
    '        <button',
    '          x-show="canEdit"',
    '          x-cloak',
    '          @click="openEdit()"',
...
'    },',
    ...
    '      return { total: total, cats: top.map(function(c){ return { name: c, count: cats[c] }; }) };',
    '    },',
    '',
...
function page_(title, body) {
  return [
    ...
    '  <style>',
    '    body {',
    '      background: radial-gradient(circle at 5% 0%, #e8f3e8 0, transparent 26%),',
    '                  radial-gradient(circle at 95% 0%, #eef6e9 0, transparent 25%),',
    '                  #f4f7f2;',
